...
extern mclComponentData __MCC_strip_infer_tool_component_data;
...
#ifndef LIB_strip_infer_tool_C_API
#define LIB_strip_infer_tool_C_API /* No special import/export declaration */
#endif

LIB_strip_infer_tool_C_API 
bool MW_CALL_CONV strip_infer_toolInitializeWithHandlers(
  ...
  if (!mclmcrInitialize())
    return false;
    {
        mclCtfStream ctfStream = 
            mclGetEmbeddedCtfStream((void *)(strip_infer_toolInitializeWithHandlers), 
                                    183808);
        if (ctfStream) {
            bResult = mclInitializeComponentInstanceEmbedded(   &_mcr_inst,
                                                                
                                                     &__MCC_strip_infer_tool_component_data,
                                                                true, 
                                                                NoObjectType, 
                                                                ExeTarget,
                                                                error_handler, 
...
LIB_strip_infer_tool_C_API 
bool MW_CALL_CONV strip_infer_toolInitialize(void)
{
  return strip_infer_toolInitializeWithHandlers(mclDefaultErrorHandler, 
                                                mclDefaultPrintHandler);
}
LIB_strip_infer_tool_C_API 
void MW_CALL_CONV strip_infer_toolTerminate(void)
{
  if (_mcr_inst != NULL)
...
int run_main(int argc, const char **argv)
{
  ...
  __MCC_strip_infer_tool_component_data.path_to_component = path_to_component; 
  if (!strip_infer_toolInitialize()) {
    return -1;
  }
  argc = mclSetCmdLineUserData(mclGetID(_mcr_inst), argc, argv);
  _retval = mclMain(_mcr_inst, argc, argv, "strip_infer_tool", 0);
  if (_retval == 0 /* no error */) mclWaitForFiguresToDie(NULL);
  strip_infer_toolTerminate();
...
int main(int argc, const char **argv)
{
  if (!mclInitializeApplication(
    __MCC_strip_infer_tool_component_data.runtime_options, 
    __MCC_strip_infer_tool_component_data.runtime_option_count))
    return 0;
